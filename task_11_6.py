...
class NotEquipmentTypeError(Exception):
    pass
...
class Storehouse:
    ...
    def __wrap(func):
        def wrapper(self, *args):
            try:
                if not isinstance(args[0], Equipment):
                    raise NotEquipmentTypeError('Item class is not Equipement')
            except NotEquipmentTypeError as e:
                print(e)
    ...
    def equipment_delivery(self, equipment, equipment_num, department_name):
        try:
            if type(equipment_num) is not int:
                raise NotNumberError('Error. Number of equipent items should be integer')
...
print()
my_storehouse.equipment_delivery('ff', 1, 'department')
print()
my_storehouse.equipment_delivery(scanner_2, 'e', 'department')
print()
